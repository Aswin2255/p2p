...
let roomLogic = (ws: WebSocket) => {
  ...
  if (findRoom && findRoom.members && findRoom.members.length < 2) {
    ...
    if (sender) {
      ...
      senderSocket.send(
        JSON.stringify({ type: "create-offer", roomid: findRoom.roomId })
      );
    }
    ...
    if (message.type === "lobby") {
      ...
      if (findRoom) {
        ...
        if (reciver) {
          ...
          receiverSocket.send(
            JSON.stringify({ type: "create-answer", sdp: sdp, roomid: roomid })
          );
      ...
      if (findRoom) {
        ...
        if (sender) {
          senderSocket = sender.socket;
          senderSocket.send( 
            JSON.stringify({ type: "receiver-answer", sdp: sdp, roomid: roomid })
          );
        }
      }
